...
Endpoints (all JSON):
  ...
  ``GET  /v1/facts/{fact_id}`` → inspect one stored fact by ID
  ``POST /v1/learn``    → extract/store facts from conversation turns
  ...
  ``POST /v1/facts/resolved`` → ingest pre-structured facts through supersession
...
class LearnRequest(BaseModel):
    messages: list[LearnMessageRequest] = Field(min_length=1)
    provider: str | None = None
    api_key: str | None = None
    model: str | None = None
    event_time: str | None = Field(
        default=None,
        description="ISO-8601 real-world anchor for the whole conversation.",
    )


class ResolvedFactRequest(BaseModel):
    content: str = Field(min_length=1)
    ...
    op: str | None = Field(
        default=None,
        description="Structured memory op: add, update, delete, or noop.",
    )
    event_time: str | None = Field(
        default=None,
        description="ISO-8601 real-world anchor for this structured fact.",
    )


class ResolvedFactsRequest(BaseModel):
    facts: list[ResolvedFactRequest] = Field(min_length=1)
...
def _parse_memory_op(value: str | None) -> MemoryOp:
    """Parse a structured memory op or raise an HTTP 422."""
    raw = (value or MemoryOp.ADD.value).strip().lower()
    try:
        return MemoryOp(raw)
    except ValueError as exc:
        raise HTTPException(
            status_code=422,
            detail=f"invalid memory op {value!r}; expected one of {[op.value for op in MemoryOp]}",
        ) from exc
...
def _serialize_lineage_fact(fact: Fact, *, active_at: datetime) -> dict[str, Any]:
    """JSON-safe projection for lineage/audit views."""
    payload = _serialize_fact(fact, active_at=active_at)
    payload["slot_key"] = fact.slot_key or None
    payload["entity"] = fact.entity or None
    payload["attribute"] = fact.attribute or None
    payload["value_text"] = fact.value_text or None
    payload["version"] = fact.version
    payload["supersedes_id"] = fact.provenance.supersedes_id or None
    payload["published_by"] = fact.provenance.published_by or None
    return payload
...
def _learn_payload(kb: KnowledgeBase, req: LearnRequest) -> dict[str, Any]:
    """Run learn() with MCP-style degraded fallback when no provider is configured."""
    turns = [ConversationTurn(role=msg.role, content=msg.content) for msg in req.messages]
    anchor = _parse_dt(req.event_time)
    try:
        facts = kb.learn(
            turns,
        ...
        return {"stored": len(facts), "ids": [fact.id for fact in facts]}
    except ValueError as exc:
        if "No API key for provider" not in str(exc):
    ...
    if last_user is None:
        return {"stored": 0, "ids": [], "note": "no user message found"}
    fact = kb.add(last_user, event_time=anchor)
    return {
        "stored": 1,
        "ids": [fact.id],
...
def create_app(kb: KnowledgeBase, *, token: str | None = None) -> FastAPI:
    ...
    def get_fact(
        ...
        try:
            ...
            raise HTTPException(status_code=404, detail=f"no fact found with id {fact_id}") from exc
        return _serialize_fact(fact, active_at=anchor)

    @app.get("/v1/facts/{fact_id}/lineage")
    def get_fact_lineage(
        ...
        if not chain:
            raise HTTPException(status_code=404, detail=f"no fact found with id {fact_id}")
        return [_serialize_lineage_fact(fact, active_at=anchor) for fact in chain]
    ...
    @app.post("/v1/learn")
    def learn(req: LearnRequest, _: None = Depends(require_auth)) -> dict[str, Any]:
        """Extract and store facts from conversation turns.

        Mirrors the MCP `learn` tool: when an extraction provider is configured,
        ai-knot runs the full LLM-backed ingest path; otherwise it degrades to
        storing the last user turn verbatim so the write path stays usable.
        """
        return _learn_payload(kb, req)
    ...
    @app.post("/v1/facts/resolved", status_code=201)
    def add_resolved_facts(
        req: ResolvedFactsRequest, _: None = Depends(require_auth)
    ) -> list[dict[str, Any]]:
        """Insert pre-structured facts through supersession with no LLM call."""
        built: list[Fact] = []
        for spec in req.facts:
            fact = Fact(
                content=spec.content,
                entity=spec.entity or "",
                attribute=spec.attribute or "",
                value_text=spec.value_text or "",
                slot_key=spec.slot_key or "",
            )
            fact.op = _parse_memory_op(spec.op)
            event_time = _parse_dt(spec.event_time)
