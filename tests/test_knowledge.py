...
class TestLLMVsBaseDifferences:
    ...
    Note: The Phase E recall pipeline uses LLM query expansion only on the
    dense (embedding) side; BM25 ranking always sees the raw query. Tests in
    this class therefore focus on aspects that survive the new pipeline (e.g.
    decay-config retention math). Brittle ranking-position assertions for
    expansion / auto-tags were removed — query_expander is unit-tested in
    `tests/test_query_expander.py`, and the holistic effect of LLM features
    is measured by the LoCoMo benchmark (`research/dated_full10_analysis_*.md`).
    """
