...
def test_retriever_throughput_qps(
    ...
    """TFIDFRetriever throughput on 1 000 facts must exceed 50 QPS.

    benchmark.stats["ops"] is the built-in QPS metric (1 / mean).
    Industry context: Qdrant P99 = 38.71ms at 2200 QPS for 1M vectors;
    our TF-IDF target is more modest — single-threaded, in-memory, zero deps.
    """
    retriever = TFIDFRetriever()
    ...
    assert qps > 50, f"Throughput too low: {qps:.1f} QPS (target: >50)"
