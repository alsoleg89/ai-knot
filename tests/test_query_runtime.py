...
# ---------------------------------------------------------------------------
# Claims-first additive promotion (retrieval-bucket cat1 fix)
# ---------------------------------------------------------------------------


def test_merge_promoted_first_protects_existing_top6():
    """Additive merge: PROTECT_K=6 existing leaders stay; new IDs come after."""
    from ai_knot.query_runtime import _merge_promoted_first

    existing = [f"e{i}" for i in range(12)]  # e0..e11
    promoted = ["p0", "p1", "p2"]

    result = _merge_promoted_first(promoted, existing)

    assert result[:6] == existing[:6], "Top-6 of existing must be preserved"
    assert result[6:9] == promoted, "Promoted IDs insert after top-6"
    assert result[9:] == existing[6:], "Tail of existing follows promoted"
    assert len(result) == len(existing) + len(promoted)


def test_merge_promoted_first_skips_duplicates_in_existing():
    """Promoted IDs already present in existing must NOT duplicate; relative
    order of existing is preserved even when shifted by NEW insertions."""
    from ai_knot.query_runtime import _merge_promoted_first

    existing = ["a", "b", "c", "d", "e", "f", "g", "h"]
    promoted = ["g", "h", "new1", "new2"]  # g, h already in existing

    result = _merge_promoted_first(promoted, existing)

    # No duplicates — g / h appear exactly once.
    assert result.count("g") == 1 and result.count("h") == 1
    # Only genuinely-new promoted IDs got injected.
    assert "new1" in result and "new2" in result
    # Relative order of existing is preserved end-to-end.
    existing_positions = [result.index(x) for x in existing]
    assert existing_positions == sorted(existing_positions)
    # g stays after f (its prior neighbour in existing).
    assert result.index("g") > result.index("f")


def test_merge_promoted_first_empty_promoted_is_identity():
    """No promotions → existing unchanged."""
    from ai_knot.query_runtime import _merge_promoted_first

    existing = ["x", "y", "z"]
    assert _merge_promoted_first([], existing) == existing


def test_claim_source_promotions_respects_env_flag(monkeypatch):
    """Env var AIKNOT_CLAIMS_FIRST_PROMOTION=0 disables promotion entirely."""
    from ai_knot.query_runtime import _claim_source_promotions
    from ai_knot.query_types import AnswerSpace, QueryFrame, TruthMode

    frame = QueryFrame(
        focus_entities=("Melanie",),
        target_kind="set",
        answer_space=AnswerSpace.SET,
        temporal_scope="none",
        epistemic_mode=TruthMode.DIRECT,
        locality="entity_scope",
        evidence_regime=None,  # type: ignore[arg-type]
        focus_relation=None,
    )
    monkeypatch.setenv("AIKNOT_CLAIMS_FIRST_PROMOTION", "0")
    # Non-empty claim list, but flag off → no promotions.
    assert _claim_source_promotions([object()], frame, ("what", "melanie")) == []
