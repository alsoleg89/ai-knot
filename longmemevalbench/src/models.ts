 ...
 *
 * LongMemEval's official auto-judge is gpt-4o (>97% human agreement). The
 * answering reader mirrors the LOCOMO harness (gpt-4.1-nano) so the memory
 * system — not a stronger reader — is the lever under test.
 *
 * Use "ollama:<model>" syntax for local Ollama, e.g. "ollama:qwen2.5:7b".
 */
export const DEFAULT_JUDGE_MODEL = process.env["DEFAULT_JUDGE_MODEL"] ?? "gpt-4o";
export const DEFAULT_ANSWER_MODEL =
  process.env["DEFAULT_ANSWER_MODEL"] ?? "gpt-4.1-nano";

/**
 * Resolve a model alias string to an AI SDK LanguageModelV1.
 *
 * Supported prefixes:
 *   ollama:<name>  → Ollama via OpenAI-compat endpoint (OLLAMA_BASE_URL)
