...
| [codespaces-quickstart.md](codespaces-quickstart.md) | Exact install-free first-run flow for GitHub Codespaces: Python, browser, CLI, and TypeScript proofs. |
| [integrations.md](integrations.md) | Fast routing by ecosystem surface: CrewAI, AutoGen, OpenAI Agents SDK, PydanticAI, OpenClaw, LangChain/LangGraph, Vercel AI SDK, MCP, TS, HTTP, plus the right install path for each. |
