...
If you only need the cross-surface `add -> search -> list -> delete` loop, start
with [memory-commands.md](memory-commands.md).

- [Initialization — storage + provider](#initialization)
- [Memory types](#memory-types)
- [Conflict resolution](#conflict-resolution)
- [Structured correction with lineage](#structured-correction-with-lineage)
...
## Structured correction with lineage

When the value itself changes, do not mutate memory in place. Push a structured
fact through the same supersession pipeline `learn()` uses internally:

```python
from ai_knot import Fact, MemoryOp

first = kb.add_resolved([
...
print(first.version, current.version)  # 0, 1
print(kb.lineage(current.id))          # newest -> oldest chain
```

`MemoryOp.UPDATE` forces a new version even when structural resolution would
normally reinforce the prior fact. `MemoryOp.DELETE` closes the matched memory
without inserting a replacement. `MemoryOp.NOOP` explicitly skips
insert/mutation when the conversation only confirms what is already known.

The same `op` values are available over the public transports:

- CLI: `ai-knot add-resolved assistant "..." --op update|delete|noop`
- TypeScript / npm: `addResolved([{ ..., op: "update" }])`
...
ai-knot list    my_agent --include-inactive                   # audit history too
...
ai-knot setup claude --agent-id bot --storage sqlite --write-default-config    # merge into Claude's default config on macOS/Windows
ai-knot setup openclaw --agent-id bot --storage sqlite --write-default-config  # merge into OpenClaw's default config
ai-knot setup openclaw --agent-id bot --storage sqlite --write-config ~/.openclaw/openclaw.json
ai-knot serve-mcp my_agent --port 8765                      # Streamable HTTP MCP
...
surface, and `delete` / `forget` both remove a single fact by ID. `list` /
...
> Need remote MCP instead of stdio? Run `ai-knot serve-mcp myagent --port 8765`
> and point the client at `http://127.0.0.1:8765/mcp`.
...
# or merge directly into a non-default plain-JSON config file:
...
memory.get_all(include_inactive=True)   # audit trail / superseded history
...
> for full extraction. `update()` still returns a new current ID, but structured
> facts now flow through supersession so lineage is preserved instead of doing a
> blind delete+add. `get_all()` / `list()` return active memories by default;
> pass `include_inactive=True` when you want audit history, and use
> `memory.lineage(current_id)` when you want the by-id supersession chain.
> `OpenClawMemoryAdapter` also exposes `recall()`, `list()`, `lineage()`, and
> `forget()` aliases so the same `add/search/list/delete` loop still works if
> you prefer ai-knot's base verbs.
...
On supported platforms, `setup claude` and `setup openclaw` accept
`--write-default-config` and will resolve the standard client config path for
you. Use `--write-config <path>` when you need a non-default plain-JSON config file.
...
If your runtime accepts ordinary Python callables directly and does not need
LangChain-style tool objects, start one layer lower with:
...
If you want the LangGraph tool-flow shape first, `ai-knot` now also exposes:
...
That is the clearest first-run/debugging path: the agent sees the literal
`add/search/list/delete` loop from the README instead of one generic memory
tool. If you want the closest `ai-knot` surface to LangMem's own onboarding
shape, keep `create_manage_memory_tool(kb)` +
`create_search_memory_tool(kb, top_k=5)` instead.
If your agent already has a `fact_id` and needs targeted inspection, add
`create_get_memory_tool(kb)` or call
`create_basic_memory_tools(kb, top_k=5, include_get=True)`.
...
> for the zero-network tool-flow proof, and
> [`examples/langchain_integration.py`](../examples/langchain_integration.py)
> for the retriever / chat-memory path.

---

## LlamaIndex

Use `AiKnotLlamaIndexMemory` when you want the familiar LlamaIndex `memory=...`
shape instead of a custom tool or retriever seam.
...
- keeps short-term chat history in a primary memory object,
- recalls only the relevant ai-knot facts on `get(...)`,
- injects them as a system-style memory block ahead of the next turn,
- stays import-safe even when `llama-index-core` is not installed.

By default, writes store user messages directly as semantic facts and skip exact
active duplicates. If you want write-time fact extraction instead, enable
`extract_on_write=True` and provide a `KnowledgeBase` with a configured
provider or per-adapter `provider=...` / `api_key=...`.
...
The shortest repo-native proof is
[`examples/llamaindex_surface_demo.py`](../examples/llamaindex_surface_demo.py).
A real model-backed wiring example lives at
[`examples/llamaindex_integration.py`](../examples/llamaindex_integration.py);
for that path, install `ai-knot[llamaindex]` plus an LLM package such as
`llama-index-llms-openai`.
For a stack-specific walkthrough aimed at the LlamaIndex audience, use
[llamaindex-case-study.md](llamaindex-case-study.md).
...
> Runnable examples:
> [`examples/openai_agents_surface_demo.py`](../examples/openai_agents_surface_demo.py)
...
As of **July 1, 2026**, the official AutoGen README marks the framework as
**maintenance mode** and points new users toward Microsoft Agent Framework. That
means this adapter is best positioned for **existing AutoGen users**.
...
> Runnable examples:
> [`examples/autogen_surface_demo.py`](../examples/autogen_surface_demo.py)
> for the zero-network memory-surface proof, and
...
Direct example files still live in [`examples/`](../examples/): `quickstart.py`,
`shared_pool.py`, `coding_agent.py`, `openai_integration.py`, `multilingual.py`,
`pydanticai_surface_demo.py`, `pydanticai_integration.py`, and the newer
surface demos for CrewAI, LangGraph, OpenAI Agents SDK, and AutoGen.
